...
% WGlen = 0;
% % rad = linspace(0,r,M);
% % phi = linspace(0,phi,M);
% [rad,phi] = meshgrid(r,phi);
% Erad = cos(m.*phi).*besselj(m,g.*rad).*exp(-1i*beta*WGlen);
% besselDeri = (m./(g.*rad)).*besselj(m,g.*rad) - besselj(m+1,g.*rad);
% Ephi = sin(m.*phi).*besselDeri.*exp(-1i*beta*WGlen);
% Hrad = -sin(m.*phi).*besselDeri.*exp(-1i*beta*WGlen);
% Hphi = -cos(m.*phi).*besselj(m,g.*rad).*exp(-1i*beta*WGlen);
% Hz = sin(m.*phi).*besselj(m,g.*rad).*exp(-1i*beta*WGlen);
% 
% figure();
% quiver(rad,phi,real(Erad),real(Ephi));
% title(['Plot of front view for TE_',num2str(m),'_',num2str(n),' E-Field']);
% legend('E-Field');
% xlabel('x-dimension 0 to a');
% ylabel('y-dimension 0 to b=a/2');
% figure();
% quiver(rad,phi,real(Hrad),real(Hphi));
...
f = 2.5e+9;      % Frequency of operation 15GHz
...
figure
%surf()
quiver(rho.*cos(phi), rho.*sin(phi), E_rho,E_phi);  %surf
hold on
quiver(rho.*cos(phi), rho.*sin(phi), H_rho,H_phi);
